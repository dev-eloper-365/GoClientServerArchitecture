...
import (
	...
	"HackerServer/core/chk_connection"
...
func options() {
	...
	fmt.Println("[+] status \t:\tProvides Status of Connection")
	fmt.Println("[+] clear \t:\tClear terminal screen")
	fmt.Println("[+] diffuse \t:\tClear traces and close connection")
...
func main() {
	...
	for loopControl {
		...
		case user_input == "status":
			err = chk_connection.Check(connection)
			DisplayError(err)
			if err != nil {
				fmt.Println("[-] Session died, Fire up the server again")
				os.Exit(999)
			}
